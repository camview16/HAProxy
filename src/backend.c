...
int assign_server(struct stream *s)
{
	...
	if (conn &&
		...
		case BE_LB_LKUP_NWTREE:
			srv = fwnew_get_next_server(s->be, prev_srv);
			break;
...
int backend_parse_balance(const char **args, char **err, struct proxy *curproxy)
{
	...
	else if (!strcmp(args[0], "newbalance")) {
		curproxy->lbprm.algo &= ~BE_LB_ALGO;
		curproxy->lbprm.algo |= BE_LB_ALGO_NW;
	}
